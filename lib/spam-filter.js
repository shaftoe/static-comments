...
module.exports = {
  ...
  async spamCheck (comment, ip, app) {
    ...
    if (akismet === undefined) {
      log.debug('No Akismet config found in comment, ignoring spam check')
      return comment
    }
    ...
    log.debug('Validating comment content via Akismet service')
    log.debug(`comment_author: ${akismet.comment_author}`)
    log.debug(`comment_content: ${akismet.comment_content}`)

    let res

    try {
      res = await require('axios').post(
        `https://${akismet.key}.rest.akismet.com/1.1/comment-check`,
        require('querystring').stringify(akismet)
      )
    } catch (error) {
      throw new SpamError(error.message)
    }
