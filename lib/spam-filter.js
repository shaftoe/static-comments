...
module.exports = {
  ...
  async spamCheck (comment, ip, app) {
    ...
    if (comment?.akismet === undefined) {
      log.info('No Akismet config found in comment, ignoring spam check')
      return comment
    }
    if (comment.akismet?.key === undefined) throw new SpamError('Missing required "key" in Akismet config')
    if (comment.akismet?.blog === undefined) throw new SpamError('Missing required "blog" in Akismet config')

    comment.akismet.comment_content = comment.content
    comment.akismet.user_ip = ip

    log.info('Validating comment content via Akismet service')

    const res = await require('axios').post(
      `https://${comment.akismet.key}.rest.akismet.com/1.1/comment-check`,
      require('querystring').stringify(comment.akismet)
    )
